...
Scans a directory tree, respects .gitignore, and outputs file paths with token
counts. Uses tiktoken for token estimation.
...
try:
    ...
    print("Recommended: run with uv for automatic dependency handling:", file=sys.stderr)
    ...
    def walk(current: Path) -> None:
        ...
        if current.is_dir():
            ...
            try:
                ...
                skipped.append(
                    {"path": str(current.relative_to(root)), "reason": "permission_denied"}
                )
        ...
        if size_bytes > 1_000_000:
            skipped.append(
                {"path": rel_path, "reason": "too_large", "size_bytes": size_bytes}
            )
            return
        ...
        try:
            ...
            if tokens > max_file_tokens:
                skipped.append(
                    {"path": rel_path, "reason": "too_many_tokens", "tokens": tokens}
                )
                return
            ...
            skipped.append({"path": rel_path, "reason": f"read_error: {exc}"})
...
def format_tree(scan_result: dict, show_tokens: bool = True) -> str:
    ...
    lines.append(
        f"Total: {scan_result['total_files']} files, "
        f"{scan_result['total_tokens']:,} tokens"
    )
    lines.append("")
    ...
    def print_tree(node: dict, prefix: str = "") -> None:
        items = sorted(
            node.items(),
            key=lambda item: (
                not isinstance(item[1], dict) or "tokens" in item[1],
                item[0].lower(),
            ),
        )
...
def main() -> None:
    ...
    try:
        ...
        print(f"ERROR: Failed to load encoding '{args.encoding}': {exc}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.format == "json":
        ...
        files_sorted = sorted(result["files"], key=lambda item: item["tokens"], reverse=True)
        print(f"# {result['root']}")
        print(f"# Total: {result['total_files']} files, {result['total_tokens']:,} tokens")
        print()
        for file_info in files_sorted:
            print(f"{file_info['tokens']:>8} {file_info['path']}")
