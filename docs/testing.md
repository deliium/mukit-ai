...
- `backend/tests/test_composition_harmony_spans.py`, `test_composition_harmony_timeline.py`, `test_composition_harmony_compatibility.py`, `test_composition_reharmonization.py`, `test_harmony_routes.py`, and `test_llm_reharmonization.py` for explicit harmony ranges, timeline ops, compatibility findings, deterministic/AI preview, and `POST /harmony/reharmonize/preview`.
...
Deterministic sidecar coverage (`composition.analysis.v1`). Fixtures and golden vectors live under `backend/tests/fixtures/analysis/`.
...
../.venv/bin/python -m pytest \
  ...
  tests/test_composition_fingerprint.py \
  tests/test_composition_analysis_context.py \
  tests/test_composition_tonality.py \
  tests/test_composition_harmony_analysis.py \
  tests/test_composition_melody_analysis.py \
  tests/test_composition_analysis_warnings.py \
  tests/test_composition_analysis.py \
  tests/test_analysis_routes.py
```

Full backend gate (includes analysis + unrelated suites):
...
Scale / performance (bounded 100k-note class coverage; generous ceiling):

```bash
cd backend
../.venv/bin/python -m pytest tests/test_composition_analysis_scale.py tests/test_composition_analysis_golden.py
```

Secret hygiene for the analysis route:

```bash
cd backend
../.venv/bin/python -m pytest tests/test_secret_hygiene.py -k analysis
```

Set `LOG_LEVEL=DEBUG` to inspect stage timings, scope, fingerprint prefix, and warning codes. Full compositions, reports, prompts, and event arrays must never appear in logs.
...
```bash
npm run test:e2e -- e2e/analysis-panel.spec.js
```
...
Harmony editing / reharmonization journey (requires running stack with `LLM_FAKE_MODE=1`):

```bash
cd frontend && npm run test:e2e -- e2e/harmony-workflow.spec.js
```
